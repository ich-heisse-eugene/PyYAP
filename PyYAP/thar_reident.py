...
def write_disp(name, data, prihdr):
    ...
    text_file.close()
    prihdr['WLSLTN'] = (str(f_name.split('/')[-1]), 'wavelength solution')
    return(prihdr)
...
def write_features(name, features, prihdr):
    ...
    for ii in range(0, len(features)):
        text_file.write(str(int(features[ii][0]))+'\t'+str(round(features[ii][1],2))+'\t'+str(round(features[ii][2],4))+'\t'+str(round(features[ii][3],1))+'\n')
    text_file.close()
    prihdr['FEATURES'] = (str(f_name.split('/')[-1]), 'ThAr features for WLSLTN')
    return(prihdr)

####################################################################
def write_new_short(spectrum, features, prihdr, dir_name):
    text_file = open(dir_name+'thar_new.dat', "w")
    for ii in range(0, len(features)):
        text_file.write(str(int(features[ii][0]))+'\t'+str(round(features[ii][1],2))+'\t'+str(round(features[ii][2],4))+'\t'+str(round(features[ii][3],1))+'\n')
    text_file.close()
    hdu = pyfits.PrimaryHDU(spectrum)
    hdu.header = prihdr
    hdu.writeto(dir_name+'s_thar_new.fits', overwrite=True)

####################################################################
### checker
def WL_checker(WL, o, features):
    for ii in range (0, len(features)):
...
def dispers_p(features, view):#, fit_params):
    good=True
    while (good):
        ...
        errorfunction = lambda C: np.ravel(solution(C)(X,O) - W)
        C, success = optimize.leastsq(errorfunction, params, maxfev=10000)
        resudial = solution(C)(X,O)
        resudial = W-resudial
        good=False
        if np.max(np.absolute(resudial))>tolerance/2:
            good=True
            index = np.where(np.absolute(resudial)>tolerance/2)[0]
...
####################################################################
### auto search lines
def add_lines(spectrum, new_features, thar, disp_params):
        ...
        for jj in range (int(FWHM*2),row_S.shape[0]-int(FWHM*2)):
            if (row_S[jj]-row_S[jj-1]>0 and row_S[jj+1]-row_S[jj]<0):                       #search for local extremum of smoothed spectra
                ...
                if ((y_coo-bckgrnd)>threshold):                                             #check threshold
                    PWL = solution(disp_params)(x_coo,ii+OS)
                    loc_thar = (thar - PWL)**2
                    ...
                    if np.sqrt(loc_thar[nearest]) < tolerance*2:
                        ...
                        if offset!=0:
                            ...
                            if np.sqrt(loc_thar[nearest])<tolerance:
                                WL = thar[nearest]
                                if WL_checker(WL, ii, new_features)==0:
...
def reidentify_features(s_order, zero_features, line, shift, new_features):
...
######################################################################
def first_ident(spectrum, zero, zero_features):
    a=0
    new_features=[]
    #for every order
    founded=0
...
##        shift = search_shift(s_order, z_order)  #get shift
        ...
        new_features = reidentify_features(s_order, zero_features, ii, shift, new_features)
...
####################################################################
def thar_auto(dir_name, file_name, view):
    #params
    global old_thar
    ...
    new_features=first_ident(spectrum, zero, zero_features)                 #identify and centering features from short list
    ...
    disp_params, new_features, points, rms = dispers_p (new_features, view)       #search first solution for short list
##    FWHM = FWHM/2
    new_features = add_lines(spectrum, new_features, thar, disp_params)     #add lines
##    tolerance = tolerance/2
    disp_params, new_features, points, rms = dispers_p (new_features, view)       #search solution for long list
    write_disp(file_name, disp_params, prihdr)
