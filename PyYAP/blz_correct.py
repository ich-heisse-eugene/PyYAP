...
#     extract_blz(argv[1], "legendre", 9, 10, 1., 6.)
    # wl, fl, hdr = read_multispec(argv[1])
    # wl_s, fl_s, hdr_s = read_multispec(argv[2])
    # cont = np.zeros(np.shape(wl))
    # blz = np.zeros(np.shape(wl))
    # nord = np.shape(wl)[0]
    # for i in range(nord):
    #     _, cont[i, :] = normalize_iraf(wl[i, :], fl[i, :], "legendre", 9, 10, 1., 6.)
    #     blz[i, :] = cont[i, :] / np.max(cont[i, :])
    #     # plt.plot(wl[i, :], cont/np.max(cont), 'r-', lw=0.5)
    # plt.plot(wl, fl_s/(cont), 'k.', ms=0.5)
    # plt.show()
