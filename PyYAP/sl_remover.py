...
def sl_remover(dir_name, Path2Temp, file_name, ap_file, step, x_order, y_order, subtract, view):
    ...
    orders = np.asarray(orders) #array with centers of orders

    X = []
    # X.append(orders[0,:]-(orders[1,:]-orders[0,:])/2.)
    ...
    scatter_file = Path2Temp +'/'+  os.path.splitext(file_name)[0] +  '_SLMap.fits'
    ...
    cleared_file = dir_name +'/'+ os.path.splitext(file_name)[0] + '_SLR.fits'
    if subtract:
...
# Path2Temp='/home/eugene/pipelines/PyYAP/20200112/temp/'
# file_name = 's_flat.fits'
...
# view = True
#
# sl_remover(dir_name, Path2Temp, file_name, ap_file, step, x_order, y_order, subtract, view)
