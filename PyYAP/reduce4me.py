...
##start here
def S_EX(conf):
    logging.basicConfig(filename=conf['Path2Data']+'/'+'pyyap_journal.log', level=logging.INFO, format='%(asctime)s %(message)s')
   ...
   ####trim overscan and flip spectra
    ...
    trimmer_data = trimmer(Path2Data, '/temp/bias_list.txt', area, flip)
    print("Biases trimmed")
    logging.info("Biases trimmed")

    trimmer_data = trimmer(Path2Data, '/temp/flat_list.txt', area, flip)
    print("Flats trimmed")
    logging.info("Flats trimmed")

    trimmer_data = trimmer(Path2Data, '/temp/thar_list.txt', area, flip)
    print("ThArs trimmed")
    logging.info("ThArs trimmed")

    trimmer_data = trimmer(Path2Data, '/temp/obj_list.txt', area, flip)
    ...
    sbias_data = medianer (Path2Data, Path2Temp + '/bias_list.txt', s_bias_name)
   ...
   ####subtract super bias from flats
    ...
    status = list_subtractor(Path2Data, Path2Temp + '/flat_list.txt', Path2Data + '/' + s_bias_name)
   ...
   ####subtract super bias from thars
    ...
    status = list_subtractor(Path2Data, Path2Temp + '/thar_list.txt', Path2Data + '/' + s_bias_name)
   ...
   ####subtract super bias from objects
    ...
    status = list_subtractor(Path2Data, Path2Temp + '/obj_CRR_cleaned_list.txt', Path2Data + '/' + s_bias_name)
    print(f"Objects: {status}")
    logging.info(f"Objects: {status}")
    print()
    ...
    sflat_data = medianer (Path2Data, Path2Temp + '/flat_list.txt', s_flat_name)
   ...
   ##make median super ThAr, delete old files
    ...
    thars = thar_combiner(Path2Data, Path2Temp + '/thar_list.txt')
    ...
    with open(Path2Temp + '/obj_CRR_cleaned_list.txt', 'r') as f:
        objects_list = f.readlines()
    f.close()
    with open(Path2Temp + '/ordim_list.txt', 'w+') as f:
        objects_list.append(Path2Data+'/'+s_flat_name)
        f.writelines(objects_list)
    f.close()
    sordim_data = medianer(Path2Data, Path2Temp + '/obj_CRR_cleaned_list.txt', s_ordim_name)
    ...
    if subtract:
        ...
        ap_file = Path2Temp + '/traces.txt' ##
        sl_remover_data = sl_remover(Path2Data, Path2Temp, s_flat_name, ap_file, step, x_order, y_order, subtract, view)
        shutil.move(Path2Data+'/'+s_flat_name, Path2Temp)
        print()

        ap_file = Path2Temp + '/traces.txt'
        f_out=open(Path2Temp+'/obj_sl_cleaned_list.txt', 'a')
        with open(Path2Temp+'/obj_CRR_cleaned_list.txt', 'r') as f:
            for line in f:
                name = line.strip()
                name = name.split('/')[-1]
                sl_remover_data = sl_remover(Path2Data, Path2Temp, name, ap_file, step, x_order, y_order, subtract, view)
                shutil.move(Path2Data +'/'+name, Path2Temp)
                f_out.write(sl_remover_data + '\n')
                print()
            f.close()
        f_out.close()
        flat_name = Path2Data + '/s_flat_SLR.fits'
        list_name = Path2Temp + '/obj_sl_cleaned_list.txt'
    else:
        flat_name = Path2Data + '/s_flat.fits'
        list_name = Path2Temp + '/obj_CRR_cleaned_list.txt'
    ...
    if ex_type != 'FOX':
        ...
        status = extract_blz(status, Path2Data + s_blaze_name, "legendre", 9, 10, 1., 6.) # Here, there's a way to improve the code
        print(f"Extracted s_blz spectrum saved in {status}")
        logging.info(f"Extracted s_blz spectrum saved in {status}")
        s_blaze_name = status

   ####params:
    out_list_name = Path2Temp + '/obj_extracted.txt'             #name of list
    out_err_list_name = Path2Temp + '/err_extracted.txt'
   ...
   ####extract ThAr
    list_name = Path2Temp + '/s_thar_list.txt'                               #name of list with images
    out_list_name = Path2Temp + '/s_thar_extracted.txt'                      #name of list
    f_out=open(out_list_name, 'a')
    with open(list_name, 'r') as f:
        for line in f:
            name = line.strip()
        ...
        try:
            ...
            for files in source:
                if files.endswith('_ec.fits'):
                    shutil.move(files, Path2Data+'/Reduced')
   ...
   ####params:
    list_name = Path2Temp + '/s_thar_extracted.txt'                  #name of list with extracted thars
    shutil.copy2(Pkg_path+'/devices/'+conf['device']+'/'+conf['device']+'_thar.dat', Path2Data+'/thar.dat')                  #copy files to working directory
    shutil.copy2(Pkg_path+'/devices/'+conf['device']+'/'+conf['device']+'_thar_last.dat', Path2Data+'/thar_last.dat')        #copy files to working directory
    shutil.copy2(Pkg_path+'/devices/'+conf['device']+'/'+conf['device']+'_thar_last.fits', Path2Data+'/thar_last.fits')      #copy files to working directory
    if not os.path.exists(Path2Data+'/Old_Thars'):
    ...
    with open(list_name, 'r') as f:
        ...
        for line in f:
           ...
           ##save new WL solution to archive
            ...
            shutil.copy(name, './Old_Thars/' + dt + '_thar.fits')
            shutil.copy(name.replace('_ec', '_err'), './Old_Thars/' + dt + '_thar_err.fits')
            shutil.copy(os.path.splitext(name)[0] + "_disp.txt", './Old_Thars/' + dt + '_thar_disp.txt')
            shutil.copy(os.path.splitext(name)[0] + "_features.txt", './Old_Thars/' + dt + '_thar_features.txt')
            print()
    f.close()
    print()

    shutil.move(flat_name, Path2Temp)
    os.remove(Path2Data + '/thar_last.dat')
    os.remove(Path2Data + '/thar.dat')
    ...
    obj_err_list = Path2Temp + '/err_extracted.txt'                      #name of list with objects
    thar_list = Path2Temp + '/s_thar_extracted.txt'                  #name of list with extracted thars

    # ThAr
    with open(thar_list, 'r') as f:
        for line in f:
            name = line.strip()
            solution = thar_manager(Path2Data, name, thar_list)
            print(f'{name}: nearest thar is '+str(solution[0].split('/')[-1])+', difference: ' + str(solution[1]) + 'sec')
            logging.info(f'{name}: nearest thar is '+str(solution[0].split('/')[-1])+', difference: ' + str(solution[1]) + 'sec')
    ...
    with open(obj_err_list, 'r') as f:
        for line in f:
            ...
            print(name)
            solution = thar_manager(Path2Data, name, thar_list)
            print(f"Nearest thar is {str(solution[0].split('/')[-1])}, difference: {str(solution[1])} sec")
            logging.info(f"Nearest thar is {str(solution[0].split('/')[-1])}, difference: {str(solution[1])} sec")
            name_cal = disp_add(name, solution[0], view)
            print()
    f.close()
    ...
    if ex_type !='FOX':
        name = flat_name.replace(".fits", "_ec.fits")
        solution = thar_manager(Path2Data, name, thar_list)
        print(f"{name}: nearest thar is {str(solution[0].split('/')[-1])}, difference: {str(solution[1])} sec")
        logging.info(f"{name}: nearest thar is {str(solution[0].split('/')[-1])}, difference: {str(solution[1])} sec")
        name_cal = disp_add(name, solution[0], view)
        ...
        obj_list = Path2Temp + '/obj_extracted.txt'
        with open(obj_list, 'r') as f:
            for line in f:
                name = line.strip()
                print(name.replace('_ec.fits', '_ec_WCS.fits'))
                out_name = name.replace('_ec.fits', '_blz_ec_WCS.fits')
                status = remove_blz(name.replace('_ec.fits', '_ec_WCS.fits'), s_blaze_name, out_name)
    ...
    if eval(conf['strip']):
        ...
        logging.info("Cleaning catalogues...")
        shutil.rmtree(Path2Data+'temp')
        shutil.rmtree(Path2Data+'Old_Thars/')
        for filepath in glob.iglob(os.path.join(Path2Data+'/', '*.*')):
            os.remove(filepath)
